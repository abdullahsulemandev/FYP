...
def get_applied_filter(video_id):
    try:
        with sqlite3.connect(DB_PATH) as conn:
            ...
            # Query to get the filter, timestamp, and user_email for a specific video_id
            query = '''
                SELECT filter, timestamp, user_email 
                FROM filters 
                WHERE video_id = ?
            '''
            ...
            if result:
                # Ensure parsing logic matches the query result
                filters = [{"filter": row[0], "timestamp": row[1], "user_email": row[2]} for row in result]
                return jsonify({"filters": filters}), 200
            else:
                return jsonify({"message": "No filters applied for this video."}), 404

    except sqlite3.Error as db_error:
        print(f"Database Error fetching applied filter: {db_error}")
        return jsonify({"error": "Database error occurred."}), 500
    except Exception as e:
        print(f"Unexpected Error fetching applied filter: {e}")
